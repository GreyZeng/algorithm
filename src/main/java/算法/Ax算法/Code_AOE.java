...
/*
 * 给定两个数组x和hp，长度都是N。 x数组一定是有序的，x[i]表示i号怪兽在x轴上的位置 hp数组不要求有序，hp[i]表示i号怪兽的血量 为了方便起见，可以认为x数组和hp数组中没有负数。
 * 再给定一个正数range，表示如果法师释放技能的范围长度(直径！) 被打到的每只怪兽损失1点血量。 返回要把所有怪兽血量清空，至少需要释放多少次aoe技能？ 三个参数：int[] x,
 * int[] hp, int range 返回：int 次数
 */
public class Code_AOE {

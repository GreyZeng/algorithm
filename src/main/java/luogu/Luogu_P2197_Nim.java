/*甲，乙两个人玩 Nim 取石子游戏。
        ...
        Nim 游戏的规则是这样的：地上有 nn 堆石子（每堆石子数量小于 10^410
        4
        ），每人每次可从任意一堆石子里取出任意多枚石子扔掉，可以取完，不能不取。每次只能从一堆里取。最后没石子可取的人就输了。假如甲是先手，且告诉你这 nn 堆石子的数量，他想知道是否存在先手必胜的策略。
        ...
        输出格式
        共 TT 行，如果对于这组数据存在先手必胜策略则输出 Yes，否则输出 No，每个单词一行。

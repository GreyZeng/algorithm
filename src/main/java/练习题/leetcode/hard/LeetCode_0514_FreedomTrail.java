...
public class LeetCode_0514_FreedomTrail {
  ...
  // 当前位置是curPos
  // 需要搞定的位置是index...
  private static int process(char[] keys, int curPos, int index, int size,
    ...
    for (int toPos : positions) {
      ans = Math.min(ans,
          dail(curPos, toPos, size) + 1 + process(keys, toPos, index + 1, size, dict));
    }
    return ans;
  }
  ...
  // 需要搞定的位置是index...
  private static int process(char[] keys, int curPos, int index, int size,
      Map<Character, List<Integer>> dict, int[][] dp) {
    ...
    for (int toPos : positions) {
      ans = Math.min(ans,
          dail(curPos, toPos, size) + 1 + process(keys, toPos, index + 1, size, dict, dp));
    }
    dp[curPos][index] = ans;
    return ans;
  }

...
public class Code_LightProblem {
  ...
  public static int p(
      int firstStatus, int endStatus, int next, int curStatus, int preStatus, int[] arr) {
    if (next == arr.length) { // 最后一按钮！
      return (endStatus != firstStatus || endStatus != preStatus)
          ? Integer.MAX_VALUE
          : (endStatus ^ 1);
    }
    if (preStatus == 0) {
      ...
      int p1 =
          p(
              firstStatus,
              next == arr.length - 1 ? endStatus ^ 1 : endStatus,
              next + 1,
              arr[next] ^ 1,
              curStatus ^ 1,
              arr);
  ...
  public static int traceLoop(
      int[] arr, int preStatus, int curStatus, int endStatus, int firstStatus) {
    int i = 3;
    int op = 0;

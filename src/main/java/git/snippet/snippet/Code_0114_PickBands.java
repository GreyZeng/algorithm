...
//
// 数据量猜解法
// program先做一次过滤，因为根据数量，最多120
//
// 用整数来判断哪个乐队挑了，哪个乐队没挑过
//
// C(120,8)就是递归的复杂度 ---》超过10的8次方，需要优化
//
// 分治
//
// 任意八个球队的状态获取到，取反，就是另外八只球队
// C(120,4) = 8 * 10^6
//
// 存任意八个球队的状态，取反，去map找另外的状态
public class Code_0114_PickBands {
  ...
  public static void f(
      int[][] programs, int size, int index, int status, int cost, int rest, int[] map) {
    if (rest == 0) {

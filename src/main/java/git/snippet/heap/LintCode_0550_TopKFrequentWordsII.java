/*
 * 550. 最常使用的K个单词II 在实时数据流中找到最常使用的k个单词. 实现TopK类中的三个方法: TopK(k), 构造方法 add(word), 增加一个新单词 topk(),
 * 得到当前最常使用的k个单词.
 *
 * 样例 样例 1:
 *
 * 输入： TopK(2) add("lint") add("code") add("code") topk() 输出：["code", "lint"] 解释： "code" 出现两次并且
 * "lint" 出现一次， 它们是出现最频繁的两个单词。 样例 2:
 *

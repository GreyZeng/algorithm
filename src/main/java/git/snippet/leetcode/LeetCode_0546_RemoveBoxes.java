...
public class LeetCode_0546_RemoveBoxes {
  ...
  public static int process1(int[] boxes, int L, int R, int K, int[][][] dp) {
    ...
    for (int i = L + 1; i <= R; i++) {
      if (boxes[i] == boxes[L]) {
        ans =
            Math.max(ans, process1(boxes, L + 1, i - 1, 0, dp) + process1(boxes, i, R, K + 1, dp));
      }
    }
    dp[L][R][K] = ans;
    return ans;
  ...
  public static int process2(int[] boxes, int L, int R, int K, int[][][] dp) {
    ...
    for (int i = last + 2; i <= R; i++) {
      if (boxes[i] == boxes[L] && boxes[i - 1] != boxes[L]) {
        ans =
            Math.max(
                ans, process2(boxes, last + 1, i - 1, 0, dp) + process2(boxes, i, R, pre + 1, dp));
      }
    }
    dp[L][R][K] = ans;
    return ans;
